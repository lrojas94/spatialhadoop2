...
public class Overlaps {
  ...
  /**Class logger*/
  private static final Log LOG = LogFactory.getLog(Overlaps.class);
   ...
   *
   */
  public static class OverlapMap extends MapReduceBase
  implements
  ...
  public static class OverlapReduce<S extends Shape> extends MapReduceBase implements
  Reducer<IntWritable, IndexedText, S, S> {
	 /**Class logger*/
	 private static final Log equalsLog = LogFactory.getLog(OverlapReduce.class);
                ...
                if(isSpatialJoinOutputRequired){
                  try {
                	if(x instanceof TigerShape && y instanceof TigerShape){
                		...
                		if(t !=null && t2 != null && t.geom.overlaps(t2.geom))
                			output.collect(x,y);
                		return;
                	}
                  } catch (IOException e) {
  ...
  private static void printUsage() {
    System.out.println("Performs Overlap operation on two WKT files.");
